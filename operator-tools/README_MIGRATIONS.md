...
| `repoint_atmosphere_assets` | Point S2 L2A `AOT_10m`/`WVP_10m` `href` + `alternate.s3.href` at the **store root** instead of the `…/r10m/aot\|wvp` array so titiler can open them (`assets=AOT_10m\|variables=/quality/atmosphere/r10m:aot`). The root is used rather than the `…/quality/atmosphere` group because only the root and `measurements/reflectance` carry consolidated metadata, and over HTTP an unconsolidated group opens empty. Suffix-based ⇒ host-agnostic and idempotent; an asset is rewritten only when both hrefs match, unrecognised hrefs are logged and skipped; never touches `SCL_20m` ([titiler-eopf#163](https://github.com/EOPF-Explorer/titiler-eopf/issues/163)) |
