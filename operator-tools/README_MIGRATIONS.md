...
| `repoint_atmosphere_assets` | Point S2 L2A `AOT_10m`/`WVP_10m` **`href` only** at the **store root** (with its trailing slash) instead of the `…/r10m/aot\|wvp` array so titiler can open them (`assets=AOT_10m\|variables=/quality/atmosphere/r10m:aot`). The root is used rather than the `…/quality/atmosphere` group because only the root and `measurements/reflectance` carry consolidated metadata, and over HTTP an unconsolidated group opens empty. **`alternate.s3.href` is left on the array** — it is what `s3_item_cleanup` and `update_stac_storage_tier` consume, and they want the narrowest accurate prefix. Suffix-based ⇒ host-agnostic and idempotent; unrecognised hrefs, assets with no `alternate.s3.href`, and hrefs that would not strip to a `.zarr` root are logged and skipped; never touches `SCL_20m` ([titiler-eopf#163](https://github.com/EOPF-Explorer/titiler-eopf/issues/163)) |
