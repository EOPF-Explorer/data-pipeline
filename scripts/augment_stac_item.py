...
def _encode_true_color_query(rescale: str) -> str:
    ...
    pairs = [("variables", f"/measurements/reflectance/r10m/0:{band}") for band in _TRUE_COLOR_BANDS]
...
def _read_geozarr_spatial_metadata(item: Item, *, verbose: bool = False) -> None:
    ...
    try:
        ...
        if transform or shape:
            extra = geozarr_asset.extra_fields if isinstance(geozarr_asset.extra_fields, dict) else {}
...
def _ensure_preview_projection(item: Item) -> None:
    ...
    if getattr(proj_ext, "code", None) != candidate_code:
        try:
            ...
            warn("unable to assign proj:code " f"'{candidate_code}' for preview asset '{preview_key}': {exc}")
