...
### Task 5 — Shard the `conditions/gamma_area_*` arrays (data-model / `eopf_geozarr`)  <✅ SHIPPED — data-model PR #197 (stacked on #180)>

> **Shipped 2026-06-19** as data-model **PR #197** (`feat/s1-gamma-area-sharding` → base `feat/s1-rtc-stac-builder`, i.e. merges into #180). Change = `shards=(h, w)` on the condition `create_array` in `ingest_s1tiling_conditions` (shards gamma_area/lia/incidence_angle through the one shared write path). Real-S3 validated: object collapse 100→1 (prod ~900→1), PUT 1.7× even with concurrency, divisibility valid at 10980², byte-identical reads; +2 tests, 102 broad S1 tests green. Spec: data-model `claude-docs/specs/s1_gamma_area_sharding.md`. Honest caveat: full-array reads are not faster sharded — the win is object-count (upload + listing) + windowed reads. Old cubes need re-ingest (not auto-migrated).
