...
- I-8 **resolved (2026-06-02; pipeline shipped 2026-06-03)**: S1 stores go to per-environment buckets,
  key `{collection}/s1-grd-rtc-{tile}.zarr`. The store **key prefix is the STAC collection** and the
  **store filename stays `s1-grd-rtc-{tile}.zarr`** (NOT `s1-rtc-{tile}.zarr`: upstream
  `build_s1_rtc_stac_item` derives the item id by stripping `s1-grd-rtc-` from the filename, so renaming
  would break the id; with Option A the filename is irrelevant to rendering). Pipeline change:
  `run_ingest_register.py` now derives the prefix from `--collection` and the separate `--s3-output-prefix`
  arg was removed (also dropped from `register_v1_s1_rtc.py`). Per-environment invocations:

  | Env | `--collection` | `--s3-output-bucket` | Resulting store key |
  |-----|----------------|----------------------|---------------------|
  | local | `sentinel-1-grd-rtc-tests` | `esa-zarr-sentinel-explorer-tests` | `…-tests/sentinel-1-grd-rtc-tests/s1-grd-rtc-{tile}.zarr` |
  | staging | `sentinel-1-grd-rtc-staging` | `esa-zarr-sentinel-explorer-s1-l1grd-staging` | `…-staging/sentinel-1-grd-rtc-staging/s1-grd-rtc-{tile}.zarr` |
  | prod | `sentinel-1-grd-rtc` | `esa-zarr-sentinel-explorer-s1-l1grd-prod` | `…-prod/sentinel-1-grd-rtc/s1-grd-rtc-{tile}.zarr` |

  **Blocker for staging/prod render:** TiTiler must resolve the store from the STAC asset href
  (titiler-eopf `DatasetPathParams` Option-A change) — platform-deploy / Emmanuel. Local `-tests` runs
  are validation-only (never read by TiTiler).
