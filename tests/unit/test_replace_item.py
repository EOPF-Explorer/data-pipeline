...
class TestManageItemReplaceItemHelper:
    ...
    def test_builds_the_body_without_resolving_the_root_link(self, no_network):
        ...
        Most callers pass an item from ``Item.from_dict(...)`` with no ``root=``, so the root
        link is unresolved and pystac's default ``to_dict()`` would fetch it through its own IO.
        """
