...
def _make_mock_search(items_dicts: list, total: int | None = None) -> MagicMock:
    ...
    # run_migration consumes raw dict pages (robust to items pystac can't model); clone/fetch
    # still use .pages()/.items.
    mock_search.pages_as_dicts.return_value = (
        [{"features": list(items_dicts)}] if items_dicts else []
    )
    return mock_search
...
class TestSTACMigrationRunner:
    ...
    def test_unmodelable_item_failed_without_write(self):
        ...
        # cannot be turned into a transaction-valid write body — so it must be reported failed
        # and never written (a PUT the API is guaranteed to 400 would only burn write budget).
        runner = self._make_runner()  # _update_item (atomic PUT) is a MagicMock
        hrefless = {
            "id": "s1-rtc-30TWQ",
        ...
        assert result.items_modified == 0
        assert result.items_failed == 1
        runner._update_item.assert_not_called()  # never write a body pystac can't model
...
class TestRestampExpiresMigration:
    def test_registered_with_reporter_and_reset(self) -> None:
        ...
        assert m.reset is restamp_reset_histogram
...
def _dirty_item(i: int) -> dict:
    ...
    Carries the keys pystac needs: the write path round-trips the body through
    pystac (_transaction_body) and refuses to write items it cannot model.
    """
    return {
        "type": "Feature",
        "stac_version": "1.0.0",
...
def _multi_page_search(pages_of_items: list[list[dict]], total: int) -> MagicMock:
    """A search yielding several raw-dict pages — the real shape of a large backfill."""
    search = MagicMock()
...
class TestMaxWrites:
    ...
    def test_unmodelable_items_spend_no_budget_and_do_not_trip_the_breaker(self) -> None:
        """A body-build failure (pystac can't model the item) attempts no write, so it
        must neither consume --max-writes budget nor count toward the circuit breaker —
        both exist to bound WRITE damage, and no write happens. Pins the interaction
        between the raw-dict/_transaction_body path and the budget/breaker machinery."""
        runner, written = self._runner_recording()
        # Modified by fix_zarr_media_type (dirty type) but unmodelable (asset has no href).
        unmodelable = [
            {
                "id": f"bad-{i}",
                "assets": {"data": {"type": "application/vnd+zarr"}},
                "links": [],
            }
            for i in range(3)
        ]
        ...
        with patch("_migrate_catalog.runner.Client") as mock_client:
            ...
            result = runner.run_migration(
                ...
                fix_zarr_media_type,
                "fix_zarr_media_type",
                max_consecutive_failures=1,  # would abort if body failures counted as writes
                max_writes=2,
            )

        assert written == ["item-0"], "the modelable item must still be written"
        assert result.items_failed == 3
        assert result.items_modified == 1
        assert result.aborted is False, "body-build failures must not trip the breaker"
        assert result.reached_max_writes is False, "only 1 of 2 budgeted writes was attempted"
